...
export const Checkout: React.FC<CheckoutProps> = ({ cart, onRemove }) => {
  const subtotal = cart.reduce((acc, item) => acc + (item.price * item.quantity), 0);
  const tax = subtotal * 0.18;

  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <h1 className="text-4xl font-display font-black italic mb-12">Güvenli Ödeme</h1>
          ...
          <section className="bg-white dark:bg-stone-950 p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                ...
                <input type="text" className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 focus:ring-2 focus:ring-primary" placeholder="Mehmet Yılmaz" />
              </div>
              <div>
                <label className="block text-sm font-bold mb-2 text-stone-500 uppercase tracking-widest text-[10px]">E-posta</label>
                <input type="email" className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 focus:ring-2 focus:ring-primary" placeholder="mehmet@example.com" />
              </div>
              <div>
                <label className="block text-sm font-bold mb-2 text-stone-500 uppercase tracking-widest text-[10px]">Telefon</label>
                <input type="tel" className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 focus:ring-2 focus:ring-primary" placeholder="+90 5XX XXX XX XX" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-bold mb-2 text-stone-500 uppercase tracking-widest text-[10px]">Adres</label>
                <textarea rows={3} className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 focus:ring-2 focus:ring-primary" placeholder="Mahalle, sokak, bina ve kapı no..."></textarea>
              </div>
            </div>
          </section>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <div className="relative">
                    <input type="text" className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 font-bold" placeholder="0000 0000 0000 0000" />
                ...
                <div>
                  ...
                  <input type="text" className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 font-bold" placeholder="AA/YY" />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-stone-500 mb-2">CVV</label>
                  <input type="password" className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-lg p-4 font-bold" placeholder="***" />
                </div>
              </div>
            </div>
        ...
        <aside className="lg:col-span-5 sticky top-28">
          <div className="bg-stone-950 text-white rounded-3xl overflow-hidden shadow-3xl border border-primary/10">
            ...
            <div className="p-8 space-y-8">
              <div className="space-y-6 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                {cart.map(item => (
                  <div key={item.id} className="flex gap-4 group">
                    ...
                    <button onClick={() => onRemove(item.id)} className="text-white/20 hover:text-red-500 transition-colors self-start">
              ...
              <button className="w-full bg-primary text-stone-950 font-black py-6 rounded-2xl text-xs uppercase tracking-[0.3em] hover:scale-[1.02] active:scale-95 transition-all shadow-2xl shadow-primary/20 flex items-center justify-center gap-3">
